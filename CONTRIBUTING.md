...
Thank you for your interest. Assemblash is a local-first, headless visual
document engine for humans and AI agents. This document explains what the
project wants from contributions and how you propose one.
...
document field, must say so and must give arguments for it. It must not add the
change quietly. Additive fields with defaults are the usual way to extend both.

## The one question to ask first

> Does this belong in the generic engine, or in a downstream adapter?

Assemblash stays deliberately small (PRD §4, "Non-goals"). Product-specific
behavior, brand rules, templates, and private workflows belong in adapters or
downstream applications. They do not belong in the core.
...
- **Protected content is respected** (PRD §10.2). Normal operations and AI
  adapters must not mutate `locked`, `protected`, and `readOnly` layers.
...
- **Every mutation is reversible** (PRD FR-8). This includes MCP-triggered
  writes.

## Proposing a change

1. **Open an issue first** for anything beyond a typo or a docs fix. This
   applies especially to schema changes, new layer types, new MCP tools, and
   new dependencies.
2. **Discuss scope.** Say explicitly if the change belongs in the core or in an
   adapter. Say which PRD requirement it serves.
...
Expect a slower review, and please open an issue before you write code:
...
- **Document schema changes.** These affect persistence, migration, every
  ...
  the migration path.
- **New dependencies.** Record the license before you propose it (PRD R8). We
  do not accept a dependency because its demo looks good. License terms,
  serialization behavior, export reliability, testability, and agent access
  matter more (PRD §14).
- **New MCP tools.** Read-only tools come before mutating tools. Mutating tools
  need dry-run, expected-version checks, protected-layer checks, and an undo
  transaction ID (PRD FR-13).
- **Anything that touches the filesystem boundary.** MCP tools must stay inside
  the configured project root (PRD §10.1).

## Testing expectations

Tests must run without a live cloud provider or paid API (PRD NFR-5).

We expect coverage for: document validation, layer and group operations,
...
export a PNG. **No release may be described as working until that smoke test
has run successfully on both Windows and Linux.**
...
The README's claims follow the same rule.

## Versioning

Releases follow [Semantic Versioning](https://semver.org/) (`MAJOR.MINOR.PATCH`).
The document `schemaVersion` is an independent integer: a schema change always
increments it and must ship with a migration path. This rule applies
regardless of the release version. If your change touches the document schema,
say so in the PR and include the migration. See PRD §16.1 for the full policy.
...
Assemblash is licensed under the [Apache License 2.0](LICENSE). When you submit
a contribution, you agree that it is licensed under the same terms (Apache-2.0
§5). Do not submit code that you do not have the right to license this way.
