 ...
 */
export function shapeKindOf(layer) {
    if (layer.type !== "shape")
        return null;
...
export function cornerRadiusOf(layer) {
    if (layer.type !== "shape")
        return 0;
    const shape = layer.shape;
        ...
        case "dropShadow": {
            ...
            return [
                ...
                ...(typeof colour === "string"
                    ? [{ name: "color", value: colour, kind: "color" }]
                    : []),
            ];
        }
        default:
...
export async function getPresets(project) {
    const body = await request(`/api/projects/${encodeURIComponent(project)}/presets`);
    return body.presets;
}
/**
...
export function styleOf(layer) {
    ...
    if (layer.type === "shape") {
        ...
        // to an ellipse.
        if (layer.fill)
            properties["fill"] = layer.fill;
        if (layer.stroke)
            properties["stroke"] = layer.stroke;
    }
    return properties;
}
