...
const CLIP_SHAPES = ["rect", "ellipse"];
/**
...
function clipRadiusOf(layer) {
    const clip = layer.clip;
    if (clip && typeof clip === "object" && !Array.isArray(clip)) {
        const radius = clip["cornerRadius"];
...
async function refresh() {
    ...
    requestPreview();
    void loadPresets().catch((error) => report("presets", error));
    void drawHistory().catch((error) => report("history", error));
}
...
function drawCanvasInspector() {
  ...
  `;
    ...
    const background = form.querySelector("#canvas-background");
    const picker = form.querySelector("#canvas-background-picker");
    const transparent = form.querySelector("#canvas-transparent");
    const anchorFieldset = form.querySelector(".canvas-anchor-fieldset");
    const anchorGrid = form.querySelector(".canvas-size-anchor-grid");
    const apply = form.querySelector("#canvas-apply");
    ...
    const nextBackground = () => transparent.checked ? null : background.value;
    const resizing = () => Number(width.value) !== canvas.width || Number(height.value) !== canvas.height;
    const update = () => {
    ...
    form.addEventListener("submit", (event) => {
        ...
        if (apply.disabled || !form.reportValidity())
            return;
        const chosen = form.querySelector('input[name="canvas-anchor"]:checked')?.value;
...
function drawInspector() {
    ...
    if (layer.type === "image" || layer.type === "svg") {
        ...
        if (layer.type === "image") {
            ...
            const sourceKnown = typeof sourceWidth === "number" && sourceWidth > 0 &&
    ...
    if (layer.type === "shape") {
        ...
        }), "number", undefined, "shape-stroke-width");
        if (kind === "rect") {
    ...
    if (clipShape && !clipChoices.includes(clipShape))
        clipChoices.push(clipShape);
...
function snapshotForEcho() {
    ...
    return () => {
        if (!before || state.project !== project)
            return;
        const current = state.document;
 ...
 */
function openFromQueue(project) {
 ...
 */
async function createShape(kind) {
    if (!state.project || !state.document)
        return;
    const settings = {
...
dom.addPanel.addEventListener("click", (event) => {
    ...
    const button = target.closest("[data-text-preset]");
...
function addAssetFile(file, preferredType, point) {
    ...
    void guard(preferredType === "svg" ? "add vector" : "add image", async () => {
        ...
        try {
            ...
            const { width: layerWidth, height: layerHeight } = placedAssetSize(uploaded.asset, state.document.canvas);
...
let listedProjects = "";
function projectListKey(projects) {
...
async function drawAgentsConnected() {
    if (!agentsConnectedKnown)
        return;
    try {
...
function stillTyping() {
    if (typingIn instanceof Node && !typingIn.isConnected)
        typingIn = null;
    return typingIn !== null;
}
function followIsQuiet() {
...
async function followOtherClients() {
    ...
    try {
        ...
        if (projectListKey(projects) !== listedProjects && document.activeElement !== dom.projects) {
            fillProjectOptions(projects, query);
            if (!state.project)
                void drawRecents().catch(() => undefined);
        }
        const project = state.project;
        const listed = projects.find((one) => one.id === project);
        const current = state.document;
        if (!project || !listed || !current)
            return;
        ...
        if (listed.documentId !== current.id || listed.version === api.versionOf(current))
            return;
        void guard("follow", async () => {
            // Checked again inside the queue: an action that ran first has already
            // read the newest document.
            if (state.project !== project || state.drag || state.editingText)
                return;
            await refresh();
        });
    }
