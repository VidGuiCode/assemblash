...
export function mountTemplates(host) {
    const dom = {
        panel: el("templates"),
        form: el("slot-form"),
        variantName: el("variant-name"),
        preview: el("preview-variant"),
        addRow: el("add-variant"),
        loadValues: el("load-values"),
        valuesFile: el("values-file"),
        clearBatch: el("clear-batch"),
        renderBatch: el("render-batch"),
        rows: el("variant-rows"),
        gallery: el("gallery"),
        imageFile: el("slot-image-file"),
    };
     ...
     */
    async function drawGallery(project, rendered) {
        releaseGallery();
    ...
    async function render(what, variants) {
        await host.guard(what, async () => {
            ...
            host.say(formatCount("templates.renderedCount", rendered.variants.length, { what, version: formatNumber(rendered.templateVersion) }));
        });
    }
    ...
    dom.valuesFile.addEventListener("change", () => {
        ...
        void host.guard(t("templates.loadValues"), async () => {
            ...
            if (!Array.isArray(parsed))
                throw new Error(t("templates.expectedArray"));
            const loaded = [];
    ...
    window.addEventListener("assemblash:localechange", () => {
        drawRows();
        for (const figure of dom.gallery.querySelectorAll(".variant")) {
            const image = figure.querySelector("img");
            const name = figure.querySelector("figcaption strong")?.textContent;
            if (image && name)
                image.alt = t("templates.variantAlt", { name });
        }
    });
