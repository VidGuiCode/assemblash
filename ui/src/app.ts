...
const dom = {
  ...
  addToggle: el<HTMLButtonElement>("add-toggle"),
...
async function refresh(): Promise<void> {
  ...
  dom.canvasImage.src = await api.imageObjectUrl(
    api.pngUrl(state.project, api.versionOf(doc), interactivePreviewScale()),
  );
...
function drawInspectorLegacy(): void {
  ...
  field(dom.inspector, "Layer", layer.name ?? layer.type, (next) =>
    next === (layer.name ?? layer.type)
      ? null
      : ({ op: "rename", id: layer.id, name: next || undefined } as Operation),
  );
  if (layer.type === "text") {
    field(dom.inspector, "Text", layer.text, (next) =>
      ({ op: "update", id: layer.id, text: next }) as Operation,
    );
    field(
      dom.inspector,
      "Size",
    ...
    for (const [value, iconName] of [
      ...
      if ((layer.align ?? "left") === value) button.classList.add("selected");
      const icon = document.createElement("i");
      icon.className = `ph ${iconName}`;
      icon.setAttribute("aria-hidden", "true");
      button.append(icon);
      button.addEventListener("click", () => {
        if ((layer.align ?? "left") === value) return;
  ...
  field(
    ...
    "Width",
    String(t.width),
    (next) => resizeTo(layer, Number(next), t.height),
    "number",
  );
  field(
    dom.inspector,
    "Height",
    String(t.height),
    (next) => resizeTo(layer, t.width, Number(next)),
    "number",
  );
  ...
  field(dom.advancedInspector, "Name", layer.name ?? "", (next) =>
    next === (layer.name ?? "")
      ? null
      : ({ op: "rename", id: layer.id, name: next || undefined } as Operation),
  );
  ...
  if (layer.type === "text") {
    ...
    field(dom.advancedInspector, "Text", layer.text, (next) =>
      ({ op: "update", id: layer.id, text: next }) as Operation,
    );
    field(dom.advancedInspector, "Font", layer.fontFamily, (next) =>
      ({ op: "update", id: layer.id, fontFamily: next }) as Operation,
    );
    field(
      dom.advancedInspector,
      "Font size",
  ...
  lockBox.addEventListener("change", () => {
    if (lockBox.checked === (layer.locked ?? false)) return;
...
function drawInspector(): void {
  ...
  if (layer?.type === "text") {
    dom.inspector.append(
      action("Edit text", "ph-pencil-simple", () => beginInlineTextEdit(layer), guarded, "toolbar-action edit-text-button"),
    );
    ...
    fontInput.addEventListener("change", () => {
      if (fontInput.value !== layer.fontFamily) {
        void send("change font", { op: "update", id: layer.id, fontFamily: fontInput.value } as Operation);
      }
    });
  ...
  dom.inspector.append(
    action("Centre horizontally", "ph-align-center-horizontal", () => {
    ...
    action("Position", "ph-bounding-box", () => togglePositionPopover(), false, "toolbar-action position-button"),
    action("Group", "ph-stack", () => {
      if (ids.length > 1) void send("group", { op: "group", ids } as Operation);
    }, guarded || ids.length < 2),
    action("Duplicate", "ph-copy", () => {
  ...
  appendFieldToGrid("Rotation", String(t.rotation ?? 0), (next) => ({ op: "rotate", id: layer.id, degrees: Number(next) }) as Operation);
  appendFieldToGrid("Opacity", String(layer.opacity ?? 1), (next) => ({ op: "update", id: layer.id, opacity: Number(next) }) as Operation);
...
function openContextMenu(x: number, y: number): void {
  ...
  const add = (label: string, icon: string, run: () => void, disabled = false): void => {
    const button = document.createElement("button");
    button.type = "button";
  ...
  add("Cut", "ph-scissors", () => { if (copySelection()) deleteSelection("cut layers"); }, !editable);
  add("Copy", "ph-copy", () => { copySelection(); }, layers.length === 0);
  add("Paste", "ph-clipboard-text", pasteClipboard, !clipboardLayers());
  add("Duplicate", "ph-copy-simple", () => void sendBatch("duplicate", layers.map((layer) => ({ op: "duplicate", id: layer.id } as Operation))), !editable);
  ...
  divider();
  add("Group", "ph-stack", () => void send("group", { op: "group", ids: layers.map((layer) => layer.id) } as Operation), layers.length < 2 || !editable);
  add("Ungroup", "ph-stack-minus", () => { if (one) void send("ungroup", { op: "ungroup", id: one.id } as Operation); }, one?.type !== "group" || !editable);
  ...
  add("Distribute horizontally", "ph-columns", () => void send("distribute horizontally", { op: "distribute", ids: layers.map((layer) => layer.id), axis: "horizontal" } as Operation), !editable || layers.length < 3);
  add("Distribute vertically", "ph-rows", () => void send("distribute vertically", { op: "distribute", ids: layers.map((layer) => layer.id), axis: "vertical" } as Operation), !editable || layers.length < 3);
...
async function openProject(): Promise<void> {
  try {
    ...
    const confirmed = window.confirm(
      `This project was left locked by process ${pid}. ` +
...
function showAddSection(id?: string, active = dom.addToggle): void {
  ...
  for (const [sectionId] of addSections) {
    ...
    if (section) section.hidden = Boolean(id && sectionId !== id);
  }
...
dom.addToggle.addEventListener("click", () => {
  if (dom.addPanel.classList.contains("collapsed")) showAddSection();
  else closeAddPanel();
});
...
dom.overlay.addEventListener("pointerdown", (event) => {
  if (event.target !== dom.overlay || event.button !== 0 || !state.document) return;
  event.preventDefault();
  const canvasRect = dom.canvas.getBoundingClientRect();
  const start = { x: event.clientX, y: event.clientY };
