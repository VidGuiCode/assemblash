...
import type { Document, FontRecord } from "./api.js";
...
export interface Host {
  ...
  /** The families the store now holds, for the shared suggestion list. */
  familiesChanged(families: readonly string[]): void;
}
...
export function mountFonts(host: Host): FontPanel {
  ...
  dom.file.addEventListener("change", () => {
    ...
    void host.guard("import font", async () => {
      ...
      if (added) {
        host.say(
          `imported ${added} font file${added === 1 ? "" : "s"}${refused ? `, ${refused} refused` : ""}`,
        );
      } else {
  ...
  dom.list.addEventListener("click", (event) => {
    ...
    const confirmed = window.confirm(
      `Remove ${family} from the font store?\n\n` +
