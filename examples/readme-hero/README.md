...
This is the editable Assemblash project behind the hero image in the main
README. It is a real document, not a design mockup or a generated image, and it
was built with the 1.6.0 shape layers and drop shadow.

The picture says what the engine is without words: one document in
...
Behind the current sheet are two earlier sheets: the history that makes every
change undoable.

The layer tree is grouped the way the picture is read. Each part can be moved,
hidden or restyled as a unit:
...
The three icons are hand-drawn SVGs, sanitized on import like any other asset.
...
a family and nothing else). It ships under the same OFL licence, copied beside
it. The operation history is included: the layers were created over the command
...
The README image is the `--scale 3` export, 3600×2025. The committed project
contains the document, its sanitized SVG assets, its history, and the one
derived bold face it needs. Noto Sans itself and the rendered output stay
outside the project.
